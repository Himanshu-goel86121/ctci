def longest_palindrome(input_str):
    ...
    for i in range(len(input_str)):
        ...
        if i - min_odd_pad >= 0 and i + min_odd_pad < len(input_str) and input_str[i - min_odd_pad] == input_str[i + min_odd_pad]:
    ...
    for i in range(len(input_str) - 1):
        if input_str[i] == input_str[i + 1]:
            ...
            continue
        if i - min_even_pad >= 0 and i + min_even_pad + 1 < len(input_str) and input_str[i - min_even_pad] == input_str[i + min_even_pad + 1]:
            for j in range(1, len(input_str)):
                if i - j < 0:
                    break
